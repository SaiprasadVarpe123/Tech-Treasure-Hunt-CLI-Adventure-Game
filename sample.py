...
def run_case(case_number):
    riddles = {
        1: (
            f"{Fore.GREEN}I'm a quiet place where minds grow bright,",
            "Books surround you left and right.",
            f"Enter the password hidden in this sight.{Style.RESET_ALL}"
        ),
        2: (
            f"{Fore.GREEN}When energy is low and minds need a boost,",
            "This magic potion helps students the most.",
            f"Find the secret word where snacks are produced!{Style.RESET_ALL}"
        ),
        3: (
            f"{Fore.GREEN}Tables, keys, and queries galore,",
            "In this lab, data we store.",
            f"Find the password where SQL rules the floor!{Style.RESET_ALL}"
        ),
        4: (
            f"{Fore.GREEN}Nature's code runs wild and free,",
            "Among flowers, grass, and every tree.",
            f"Find the secret where butterflies decree!{Style.RESET_ALL}"
        ),
        5: (
            f"{Fore.GREEN}Solve me to find your next location for ",
            "200+4*300*(2+1)+(53-900*2/4)-400*3/6",
            "And find the answer that leads you to Victory!"
        )
    }
